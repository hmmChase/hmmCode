...
const checkArithmeticSubarrays = (nums, l, r) => {
  ...
  for (let i = 0; i < l.length; i++) {
    // BigO(n(m log m+m))
    const arithQueriesArr = nums.slice(l[i], r[i] + 1);

    arithQueriesArr.sort((a, b) => a - b); // BigO(n log n)

    const rangeDiff = arithQueriesArr[1] - arithQueriesArr[0];

    let isArithmetic = true;

    // BigO(n)
    for (let j = 0; j < arithQueriesArr.length - 1; j++) {
      if (arithQueriesArr[j + 1] - arithQueriesArr[j] !== rangeDiff) {
        isArithmetic = false;
